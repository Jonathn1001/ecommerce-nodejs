...
const ev = (type: string, payload: object): EventEnvelope =>
  makeEnvelope({ type, version: 1, traceId: "t", producer: "catalog", payload });
const row = (id: string) =>
  prisma.catalogReadModel.findUnique({ where: { productId: id } });
...
describe("order catalog projection (integration)", () => {
  ...
  it("product_created inserts; higher-version update applies; lower/equal version no-ops", async () => {
    ...
    await handleCatalogEvent(
      ev(CATALOG_PRODUCT_CREATED, { productId: id, name: "A", price: 100, version: 1 })
    );
    expect((await row(id))!.price).toBe(100);
    await handleCatalogEvent(
      ev(CATALOG_PRODUCT_UPDATED, { productId: id, name: "B", price: 200, version: 2 })
    );
    expect((await row(id))!.price).toBe(200);
    // stale/duplicate (version 1) -> ignored
    await handleCatalogEvent(
      ev(CATALOG_PRODUCT_UPDATED, { productId: id, name: "OLD", price: 999, version: 1 })
    );
  ...
  it("price_changed is ignored by the read model", async () => {
    ...
    await handleCatalogEvent(
      ev(CATALOG_PRICE_CHANGED, { productId: id, price: 555, version: 2 })
    );
