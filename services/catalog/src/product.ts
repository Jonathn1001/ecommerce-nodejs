...
export interface ProductWriteTx {
  ...
  loadForUpdate(
    id: string
  ): Promise<{ type: string; name?: string; price: number } | null>;
  updateProduct(
    id: string,
    data: { name?: string; price?: number; attributes?: unknown }
  ): Promise<{ name?: string; price: number; version: number }>;
...
export async function applyCreate(
  ...
  const { id, version } = await tx.createProduct({
    type: p.type,
    name: p.name,
    price: p.price,
    attributes: attrs.value,
  });
  await tx.enqueue(CATALOG_PRODUCT_CREATED, id, {
    productId: id,
    name: p.name,
    price: p.price,
    version,
...
export async function applyUpdate(
  ...
  const after = await tx.updateProduct(p.id, {
    name: p.name,
    price: p.price,
    attributes: p.attributes,
  });
  await tx.enqueue(CATALOG_PRODUCT_UPDATED, p.id, {
    productId: p.id,
    name: after.name,
    price: after.price,
  ...
  if (priceChanged) {
    await tx.enqueue(CATALOG_PRICE_CHANGED, p.id, {
      productId: p.id,
      price: after.price,
      version: after.version,
    });
