...
function fakeTx(init?: { type?: string; price?: number; exists?: boolean }) {
  ...
  const tx: ProductWriteTx = {
    ...
    async enqueue(t, _p, payload) {
      emitted.push({ type: t, payload });
    },
  };
  return { tx, emitted };
}
...
describe("applyCreate", () => {
  it("creates + emits product_created(version 1)", async () => {
    const f = fakeTx();
    const r = await applyCreate(f.tx, {
      type: "ELECTRONICS",
      name: "x",
      price: 700,
    ...
    expect(f.emitted).toEqual([
      {
        ...
        payload: { productId: "p1", name: "x", price: 700, version: 1 },
      },
    ]);
  });
    ...
    expect(f.emitted).toEqual([
      {
        ...
        payload: { productId: "p1", name: undefined, price: 900, version: 2 },
      },
      {
        type: CATALOG_PRICE_CHANGED,
        payload: { productId: "p1", price: 900, version: 2 },
      },
    ]);
  });
